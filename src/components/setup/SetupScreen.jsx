...
import { Calendar as CalendarIcon, Upload, Link as LinkIcon, Loader2, FileText, Plus, AlertCircle, Sun, Moon } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 p-6 flex flex-col items-center justify-center font-sans text-slate-800 dark:text-slate-100 transition-colors duration-300">
       <div className="absolute top-4 right-4">
          <button onClick={() => setDarkMode(!darkMode)} className="p-2 rounded-full bg-white dark:bg-slate-800 shadow-sm border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
      ...
      <div className="max-w-2xl w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="text-center space-y-2">
          <h1 className="text-4xl font-extrabold text-slate-900 dark:text-white flex items-center justify-center gap-3">
            <div className="bg-blue-600 p-2 rounded-xl"><CalendarIcon className="w-8 h-8 text-white" /></div>
            Planner
          </h1>
          <p className="text-slate-500 dark:text-slate-400 text-lg">Your academic life, organized.</p>
        </div>
        <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 grid gap-6 md:grid-cols-2">
          <div className="group flex flex-col items-center justify-center p-8 border-2 border-dashed border-slate-200 dark:border-slate-600 rounded-xl hover:bg-blue-50/50 dark:hover:bg-slate-700/50 hover:border-blue-400 dark:hover:border-blue-500 transition-all cursor-pointer relative overflow-hidden">
            <input type="file" accept=".ics" className="absolute inset-0 opacity-0 cursor-pointer" onChange={handleFileUpload} />
            <div className="bg-blue-100 dark:bg-blue-900/30 p-4 rounded-full mb-4 group-hover:scale-110 transition-transform"><Upload className="w-8 h-8 text-blue-600 dark:text-blue-400" /></div>
            <h3 className="font-semibold text-slate-700 dark:text-slate-200 mb-1">Upload Calendar</h3>
          </div>
          <div className="flex flex-col justify-center p-6 border border-slate-200 dark:border-slate-600 rounded-xl bg-slate-50/50 dark:bg-slate-900/50">
            <div className="flex items-center gap-2 mb-4"><LinkIcon className="w-5 h-5 text-purple-500" /><h3 className="font-semibold text-slate-700 dark:text-slate-200">Import via URL</h3></div>
            <div className="space-y-3">
              <input type="text" placeholder="https://..." className="w-full px-3 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm focus:ring-2 focus:ring-purple-500 outline-none dark:text-white" value={urlInput} onChange={(e) => setUrlInput(e.target.value)} />
              <button onClick={handleUrlFetch} disabled={isLoading || !urlInput} className="w-full bg-purple-600 text-white px-3 py-2.5 rounded-lg hover:bg-purple-700 disabled:opacity-50 font-medium flex items-center justify-center gap-2">
                {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Fetch Calendar'}
              </button>
            </div>
          </div>
          <div className="md:col-span-2 flex justify-center pt-2 border-t border-slate-100 dark:border-slate-700">
             <div className="flex gap-4">
               <button onClick={openJsonManual} className="text-sm text-slate-500 hover:text-blue-600 flex items-center gap-1"><FileText className="w-4 h-4" /> Manually enter JSON</button>
               <button onClick={startEmpty} className="text-sm text-slate-500 hover:text-blue-600 flex items-center gap-1"><Plus className="w-4 h-4" /> Start Empty</button>
             </div>
          </div>
        </div>
        {error && <div className="bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 p-4 rounded-xl flex items-center gap-3 border border-red-200"><AlertCircle className="w-5 h-5" />{error}</div>}
      </div>
    </div>
  );
};
