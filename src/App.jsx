...
function PlannerApp() {
  ...
  const {
    events, updateEvent, addEvent, deleteEvent, 
    classColors, setClassColors, hiddenClasses, setHiddenClasses,
    deleteClass, mergeClasses, resetAllData, exportICS, importJsonData
  } = eventsContext;

  const {
    darkMode, setDarkMode, calendarView, setCalendarView,
    view, setView, currentDate, setCurrentDate,
    searchQuery, setSearchQuery, activeTypeFilter, setActiveTypeFilter,
    showCompleted, setShowCompleted, hideOverdue, setHideOverdue,
    modals, openModal, closeModal, editingTask, setEditingTask, openTaskModal
  } = useUI();
  ...
  useEffect(() => { localStorage.setItem("hw_sync_room", roomCode); }, [roomCode]);
  ...
  const handleSyncConnect = async (code, password) => {
      try {
          ...
          // If room doesn't exist OR if it's dead, ask to Create/Takeover
          if (err.message === "Room not found" || err.message === "Room is inactive") {
              const confirmMsg = err.message === "Room not found" 
  ...
  const handleDragStart = (e, id) => { e.dataTransfer.setData("text/plain", id); setDraggedEventId(id); };
  ...
  const filteredEvents = useMemo(() => {
    ...
    return events.filter((e) => {
      ...
      if (searchQuery) return e.title.toLowerCase().includes(searchQuery.toLowerCase()) || e.class.toLowerCase().includes(searchQuery.toLowerCase());
      return true;
    });
  ...
  const renderModals = () => (
    <>
      <SettingsModal
        ...
        deleteClass={deleteClass} resetAllData={resetAllData}
        showJsonEdit={modals.jsonEdit} setShowJsonEdit={(val) => val ? openModal("jsonEdit") : closeModal("jsonEdit")}
        jsonEditText={jsonEditText} setJsonEditText={setJsonEditText} handleJsonSave={handleJsonSave} handleICSExport={exportICS}
        // Sync
        roomCode={roomCode} setRoomCode={setRoomCode}
        syncStatus={syncStatus} handleSyncConnect={handleSyncConnect} disconnectFromRoom={leaveSyncSession}
        errorMsg={errorMsg}
      />
      <TaskModal
        isOpen={modals.task} onClose={() => closeModal("task")} editingTask={editingTask}
        saveTask={(e) => {
          e.preventDefault();
          const formData = new FormData(e.target);
          const isAllDay = formData.get("isAllDay") === "on";
          const baseTask = {
            title: formData.get("title"), time: isAllDay ? "" : formData.get("time"),
            class: formData.get("class"), type: formData.get("type"),
            priority: formData.get("priority") || "Medium", description: formData.get("description") || "",
            completed: editingTask ? editingTask.completed : false,
          };
          const startDate = formData.get("date");
          if (editingTask) updateEvent({ ...baseTask, date: startDate, id: editingTask.id, groupId: editingTask.groupId });
          else addEvent({ ...baseTask, date: startDate, id: `manual-${Date.now()}`, groupId: null });
          closeModal("task");
        }}
  ...
  if (view === "setup") return <div className="bg-white dark:bg-slate-900 min-h-screen"><SetupScreen />{renderModals()}</div>;
  ...
  return (
    <div className="h-screen flex flex-col bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 font-sans overflow-hidden transition-colors duration-300">
      <header className="h-16 border-b border-slate-200 dark:border-slate-700 px-6 flex items-center justify-between bg-white dark:bg-slate-900 shrink-0 z-10">
        ...
        <div className="flex items-center bg-slate-100 dark:bg-slate-800 rounded-lg p-1">
          {[{ id: "month", icon: LayoutGrid, label: "Month" }, { id: "week", icon: Columns, label: "Week" }, { id: "day", icon: Rows, label: "Day" }, { id: "agenda", icon: AlignLeft, label: "Agenda" }].map((v) => (
            <button key={v.id} onClick={() => setCalendarView(v.id)} className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-medium transition-all ${calendarView === v.id ? "bg-white dark:bg-slate-700 text-blue-600 dark:text-blue-400 shadow-sm" : "text-slate-500 hover:text-slate-700 dark:hover:text-slate-300"}`}><v.icon className="w-4 h-4" /><span className="hidden sm:inline">{v.label}</span></button>
          ))}
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        <Sidebar searchQuery={searchQuery} setSearchQuery={setSearchQuery} activeTypeFilter={activeTypeFilter} setActiveTypeFilter={setActiveTypeFilter} hiddenClasses={hiddenClasses} setHiddenClasses={setHiddenClasses} classColors={classColors} filteredEvents={filteredEvents} hideOverdue={hideOverdue} setHideOverdue={setHideOverdue} handleDragStart={handleDragStart} handleDragOver={handleDragOver} handleSidebarDrop={handleSidebarDrop} toggleTask={(e, id) => { e.stopPropagation(); useEvents().toggleTaskCompletion(id); }} openEditTaskModal={(task) => openTaskModal(task)} draggedEventId={draggedEventId} showCompleted={showCompleted} setShowCompleted={setShowCompleted} />
        <CalendarView currentDate={currentDate} setCurrentDate={setCurrentDate} calendarView={calendarView} setCalendarView={setCalendarView} filteredEvents={filteredEvents} classColors={classColors} openEditTaskModal={(task) => openTaskModal(task)} handleDragOver={handleDragOver} handleDragStart={handleDragStart} handleDrop={handleDrop} draggedEventId={draggedEventId} />
      </div>
      {renderModals()}
    </div>
  );
...
export default function App() {
  return <EventProvider><UIProvider><PlannerApp /></UIProvider></EventProvider>;
}