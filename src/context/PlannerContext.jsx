...
export const EventProvider = ({ children }) => {
  ...
  const [changeLog, setChangeLog] = useState(() =>
    loadState("hw_change_log", []),
  );
  const [lastModified, setLastModified] = useState(() =>
    loadState("hw_last_modified", Date.now()),
  );
  ...
  useEffect(() => {
    localStorage.setItem("hw_last_modified", JSON.stringify(lastModified));
  }, [lastModified]);
  useEffect(() => {
    localStorage.setItem("hw_change_log", JSON.stringify(changeLog));
  }, [changeLog]);
  ...
  const syncWithRemote = (remoteData) => {
    const {
      changeLog: remoteLog,
      classColors: remoteColors,
      hiddenClasses: remoteHidden,
    } = remoteData;

    // 1. Merge Colors & Hidden
    if (remoteColors) setClassColors((prev) => ({ ...prev, ...remoteColors }));
    if (remoteHidden)
      setHiddenClasses((prev) => [...new Set([...prev, ...remoteHidden])]);
        ...
        newChanges.forEach((change) => {
          ...
          if (op === "ADD") {
            ...
            updated = updated.map((e) =>
              e.class === val.source ? { ...e, class: val.target } : e,
            );
